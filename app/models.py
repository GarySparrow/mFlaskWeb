...
import bleach, zlib, os
from flask import current_app, request, url_for, send_file
...
class User(UserMixin, db.Model):
    ...
    def to_json(self):
        json_user = {
            ...
            'id': self.id,
            'email': self.email,
            'gravatarURL': self.gravatar(size=256),
            ...
            'postsURL': url_for('api.get_user_posts', id=self.id, _external=True),
            'followedPostsURL': url_for('api.get_user_followed_posts',
...
class Post(db.Model):
    ...
    imgs_path = db.Column(db.PickleType)
    # imgs_data = db.Column(db.PickleType)
    comments = db.relationship('Comment', backref='post', lazy='dynamic')

    likers = db.relationship('Like',
    ...
    def to_json(self):
        ...
        root = 'http://www.garychloe.com/'
        video_path = None
        if self.video_path is not None and os.path.exists('app/' + self.video_path):
            video_path = root + self.video_path
            # if self.video_data is not None and not os.path.exists('app/' + self.video_path):
            #     file = open('app/' + self.video_path, 'w')
            #     file.write(self.video_data)

        imgs_path = []
        if self.imgs_path is not None:
            for i in range(len(self.imgs_path)):
                if os.path.exists('app/' + self.imgs_path[i]):
                    imgs_path.append(root + self.imgs_path[i])
        json_post = {
            'id': self.id,
            ...
            'authorURL': url_for('api.get_user', id=self.author_id,
                              _external=True),
            # 'gravatarURL': user.gravatar(size = 256),
            # 'authorId': self.author_id,
            'commentsURL': url_for('api.get_post_comments', id=self.id,
                                _external=True),
            'commentCount': self.comments.count(),
            'likersSum': self.likers_sum
        }
        return json_post
...
class Comment(db.Model):
    ...
    def to_json(self):
        ...
        json_comment = {
            ...
            'postURL': url_for('api.get_post', id=self.post_id, _external=True),
            # 'postId': post.id,
            'subject': self.body,
            # 'body_html': self.body_html,
            'timeStamp': self.timestamp,
